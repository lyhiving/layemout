...
function initRows() {
	...
	$('#layoutContainer .page-row').each(function() {
		...
		var widthOfPage = $('.page').width(),
			//make row height based on comic book aspect ratio
			convertToHeight = widthOfPage / .666,
			getaThird = Math.round(convertToHeight / 3);

		$(this).height(getaThird).resizable({
			handles: 's'
		}); //make the row resizable up and down
	});

}

function alertSelect() {
	alert('You gotta select a panel first, bro');
}
...
function init() { //basic page setup stuff
	...
	$('.choose').each(function() { //setting up the project template picker
		...
		$('[data-type="' + loadType + '"]').load('templates/template.html #' + loadType, function() {
			...
			if (loadType == 'page' || loadType == 'spread') {
				b = b / 2.6 + b;
			}
...
$(document).ready(function() {
	...
	$('#duplicate').click(function() {
		if ($('.selectify').length < 1) {
			...
			c.insertAfter('.selectify').html(b).resizable({
				handles: 'e, se, s'
			});
	...
	$('#loadNewRow').click(function() {
		if ($('.selectify').length < 1) {
			...
			array = [1, 2, 3, 4, 5, 6], num = Math.floor(Math.random() * array.length), roll = array.splice(num, 1);
